...
import { createBindGroup, extraBindGroup, createBindGroup_Limit } from './src/createBindGroups.js';
...
async function main() {
    ...
    async function changeString(string) {
        let { obj, Base_Vertex, Base_Normal, animationBase, limit } = await fetchData(myString);
        ...
        let { pipeline_Face, pipeline_Edge, pipeline_Vertex, pipeline_webCam, pipeline2, pipelineAnime, pipeline_Limit } = await createPipelines(device, presentationFormat);
        
        let bindGroups = [];
        for (let i=0; i<=depth; i++){
            bindGroups.push(createBindGroup(device, pipeline_Face, pipeline_Edge, pipeline_Vertex, Base_Vertex_Buffer, Base_Normal_Buffer, levels[i],i+1));
        }

        return {
            obj, Base_Vertex, Base_Normal, animationBase, limit,
            levels,
            extra_base_UVStorageBuffers,
            extra_vertex_offsetStorageBuffers,
            extra_vertex_indexesStorageBuffers,
            ...
            Base_Vertex_After_Buffer,
            OrdinaryBuffer,
            pipeline_Face, pipeline_Edge, pipeline_Vertex, pipeline_webCam, pipeline2, pipelineAnime, pipeline_Limit,
            bindGroups,
        }
    }
    ...
    async function render(now) {
        ...
        // console.log(Base_Vertex);

        device.queue.writeBuffer(Base_Vertex_Buffer, 0, Base_Vertex);
        device.queue.writeBuffer(Base_Normal_Buffer, 0, Base_Normal);
        ...
        const encoder = device.createCommandEncoder({ label: 'total encoder' });

        encoder.writeTimestamp(querySet, 0);

        if(settings.getProterty('animation')[1])
        {
            make_compute_encoder(encoder, pipelineAnime, animeBindGroup, 300, "base_vertex_buffer animation encoder");
        }

        for(let i=0; i<=depth; i++) // 등호 확인
        {
            make_compute_encoder(encoder, pipeline_Face, bindGroups[i].bindGroup_Face, 300, "face_compute_encoder");
            make_compute_encoder(encoder, pipeline_Edge, bindGroups[i].bindGroup_Edge, 300, "edge_compute_encoder");
            make_compute_encoder(encoder, pipeline_Vertex, bindGroups[i].bindGroup_Vertex, 300, "vertex_compute_encoder");
        }
        ...
        // device.queue.writeBuffer(Base_Normal_Buffer, 0, zeroData);
        ...
        // make_compute_encoder(encoder, pipeline_Limit, bindGroup_Limit, 300, 'encoder for limit position');
        ...
        //     }
        //     make_render_encoder(encoder, renderPassDescriptor, pipelines[pipelineValue], changedBindGroups[i+(depth+1)*pipelineValue], vertexBuffers[N][i], indexBuffers[N][i], narray[i] * narray[i] * 6, 'render pipeline encoder');
        // }

        let pass = encoder.beginRenderPass(renderPassDescriptor, {label: 'render pass for object'});
        pass.end();

        const OrdinaryPointfixedBindGroup = await extraBindGroup(device, uniformBuffer, OrdinaryPointData, Base_Vertex_Buffer, Base_Normal_Buffer, textures, sampler, extra_base_UVStorageBuffers, extra_vertex_offsetStorageBuffers, extra_vertex_indexesStorageBuffers, pipeline2, depth, settings)
        ...
        make_render_encoder(encoder, renderPassDescriptor2, pipeline2, OrdinaryPointfixedBindGroup, OrdinaryBuffer, false, OrdinaryPointData[settings.getProterty('ordinaryLevel')].length, 'ordinary draw encoder');
        ...
        resultBuffer.mapAsync(GPUMapMode.READ).then(() => {
            ...
            // console.log('Raw times:', times[0], times[1], typeof times[0]);
        ...
        //   gpuAverage.addSample(gpuTime / 1000); // ns → µs
        // });
...
function make_compute_encoder(encoder, pipeline, bindgroup, workgroupsize, text = " ") {
...
function make_render_encoder(encoder, renderPassDescriptor, pipeline, bindgroup, vertexBuffer, indexBuffer = false, draw_size, text = " ") {
    // const encoder = device.createCommandEncoder({label: text});
